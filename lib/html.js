...
const stripFooter = (html) =>
  html.replace(/<footer[\s\S]*?<\/footer>/gi, "");

const stripFloatingCTA = (html) =>
  html.replace(
    /<div[^>]*class=("|')[^"']*fixed bottom-8 right-8[^"']*\1[^>]*>[\s\S]*?<\/div>/gi,
    ""
  );
...
const readHtmlPageSource = (fileName) => {
  ...
  const title = titleMatch ? titleMatch[1].trim() : "Havenworks";
  ...
  bodyContent = stripFloatingCTA(bodyContent);
...
export const readHtmlPage = (fileName) => {
  ...
  const cleanedBody = stripFooter(body);
  const wrapperClass = bodyClass ? `legacy-page ${bodyClass}` : "legacy-page";
  const wrappedBody = `<div class="${wrapperClass}">${cleanedBody}</div>`;
  return { title, body: wrappedBody };
};

export const readHtmlPageBody = (fileName) => {
  const page = readHtmlPageSource(fileName);
  const cleanedBody = stripFooter(page.body);
  const repairedBody = stripFloatingCTA(cleanedBody);
  return { ...page, body: repairedBody };
};
