...
const shouldInjectSiteScripts =
  process.env.NODE_ENV === "production" ||
  process.env.NEXT_PUBLIC_ENABLE_SITE_SCRIPTS === "true";

const loadSiteScripts = async () => {
  if (!shouldInjectSiteScripts || !supabaseServer) {
...
export default async function RootLayout({ children }) {
  ...
  const safeHeadScripts = shouldInjectSiteScripts
    ? sanitizeSiteSnippet(siteScripts?.head_scripts || "")
    : "";
  const safeBodyScripts = shouldInjectSiteScripts
    ? sanitizeSiteSnippet(siteScripts?.body_scripts || "")
    : "";
